   ...
   @Published  var peripheralDisplayData: [PeripheralDisplayData] = []
    ...
    func updatePeripheralDisplayData() {
              peripheralDisplayData = discoveredPeripherals.map { (key, peripheral) in
                  let chars = peripheral.characteristics?.map { (charKey, charValue) in
                   CharacteristicDisplayData(uuid: charKey.uuidString, name: BluetoothUtils.name(for: charKey))
                   } ?? []
                return PeripheralDisplayData(id: key, name: peripheral.name ?? "Unnamed", rssi: peripheral.rssi, state: peripheral.state, characteristics: chars)
        ...
        for characteristic in characteristics {
            ...
            if characteristic.properties.contains(.notify){
               ...
               discoveredPeripherals[peripheral.identifier]?.setNotifyValue(true, for: characteristic)
                ...
                if peripheral.characteristics == nil {
...
extension CBPeripheral {
    private struct AssociatedKeys {
      static var rssi: Int = 0
       static var characteristics: [CBUUID: CBCharacteristic] = [:]
    }
    var rssi: Int {
        get {
     ...
     var characteristics: [CBUUID: CBCharacteristic]? {
            get {
                return objc_getAssociatedObject(self, &AssociatedKeys.characteristics) as? [CBUUID: CBCharacteristic]
            }
            set {
                objc_setAssociatedObject(self, &AssociatedKeys.characteristics, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
            }
        }
}
